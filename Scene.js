    ...
    for(var i = 0; i<this.sprites.length; i++){
        for(var j = i+1; j<this.sprites.length; j++){
            if(this.sprites[i].colidiuCom(this.sprites[j])){
                ...
                if(this.sprites[i].imune <= 0 && this.sprites[i].props.tipo === "pc" && (this.sprites[j].props.tipo === "npc" || this.sprites[j].tipo === "worm")){
                   this.sprites[j].vida--;
                   vidaabsoluta--;
                ...
                if((this.sprites[i].props.tipo === "npc" || this.sprites[i].props.tipo === "worm" ||this.sprites[i].props.tipo === "ghost" || this.sprites[i].props.tipo === "shark" || this.sprites[i].props.tipo === "spin") && this.sprites[j].props.tipo === "tiro"){
                    this.sprites[i].vida--;
...
Scene.prototype.checaMorte = function(){
    for(var i = 0; i < this.sprites.length; i++){
        ...
        {
            if(this.sprites[i].props.tipo != "boom")
            {  
            assetsMng.play("explosion");
...
Scene.prototype.verdados = function (){
    ...
    ctx.drawImage(this.assets.img("score"),10,60);
    ctx.fillText(pontos, 120,90);
    ctx.strokeText(pontos, 120,90);
...
Scene.prototype.stage = function (){
    if(titlecronometer <=0 && enemycronometer <=0){
        if(pontos<=50){
            for(var k=0; k < 2; k++){
               this.adicionar(new Sprite({ x: canvas.width, y: canvas.height*Math.random(), h: 20, va: 5, vm: 50, color: "blue", props:{tipo: "npc"}, comportar: persegue2(pc) }));
               enemycronometer = 1;
               stage=1;
            }
        }
            ...
            for(var k=0; k < 1+pontos*1/6; k++){
                this.adicionar(new Sprite({ x: canvas.width+48, y: canvas.height*Math.random(), w: 48, h: 16, vm: 30 + Math.random()*5, color: "lightblue", props:{tipo: "worm"}, comportar: moverworm() }));
                enemycronometer = 3;
                stage=2;
            }
        }
            ...
            for(var k=0; k < 3; k++){
                this.adicionar(new Sprite({ x: canvas.width+10, y: 300*Math.random(), w: 150, h: 80, va: -16, vm: 60, color: "pink", props:{tipo: "shark", spawn: 5}, vida: 15, comportar: persegueSpawn(pc) }));
                enemycronometer = 5;
                stage=3;
            }
        }
            ...
            for(var k=0; k < 2; k++){
                this.adicionar(new Sprite({ x: canvas.width+48, y: canvas.height*Math.random(), w: 48, h: 16, vm: 40 + Math.random()*5, color: "mediumspringgreen", props:{tipo: "worm"}, vida: 1, comportar: moverworm() }));
                this.adicionar(new Sprite({ x: canvas.width, y: canvas.height*Math.random(), h: 20, va: -16, vm: 75, color: "teal", props:{tipo: "npc"}, vida:2, comportar: persegue2(pc) }));
                enemycronometer = 1;
                stage=4;
            }
        }
            ...
            for(var k=0; k < 4; k++){
                this.adicionar(new Sprite({ x: canvas.width+48, y: canvas.height*Math.random(), w: 48, h:16, vm: 50 + Math.random()*5, color: "springreen", props:{tipo: "worm"}, vida:1, comportar: moverworm() }));
                this.adicionar(new Sprite({ x: canvas.width, y: canvas.height*Math.random(), h: 20, va: -16, vm: 150, color: "purple", props:{tipo: "npc"}, vida: 4, comportar: persegue2(pc) }));
                this.adicionar(new Sprite({ x: canvas.width+10, y: 300*Math.random(), w: 150, h: 80, va: -16, vm: 60, color: "crimson", props:{tipo: "shark", spawn: 5}, vida: 25, comportar: persegueSpawn(pc) }));
                enemycronometer = 15;
                stage=5;
            }
        }
            ...
            for(var k=0; k < 5; k++){                
                this.adicionar(new Sprite({ x: 0, y: canvas.height*Math.random(), h:50 , w: 20, vm: 300, color: "darkgray", props:{tipo: "spin"}, vida: 5}));
                enemycronometer = 2;
                stage=6;
            }
            if(boss <= 0){
              this.adicionar(new Sprite({ x: 600, y: canvas.height/8, props: {tipo: "mothership", spawn: 2}, vm: -20, vida: 300, comportar: persegueboss(pc)}));
              boss=1;
            }    
        }
    }
    if(ghostcronometer<=0 && stage >= 3){
           this.adicionar(new Sprite({ x: canvas.width/2, y: canvas.height/2, w: 60 , h: 40, va:0, vm: 30, color: "white", props:{tipo: "ghost", spawn: 2}, vida: 50, comportar: persegueSpawn2(pc) }));
           ghostcronometer = 40;
    }
};
